...
function run_experiment_pipeline(num_humans, num_simulations, write_to_file_flag = false)
    ...
    for iteration_num in 1:num_simulations
		...
		output_filename_2D_AS_planner = "./scenario_2/2D/output_expt_" * string(iteration_num) * ".txt"
		...
		if(write_to_file_flag)
			expt_details_filename_2D_AS_planner = "./scenario_2/2D/details_expt_" * string(iteration_num) * ".jld2"
	...
	results_filename = "./RESULTS/scenario2_humans_"*string(num_humans)*"_experiments_"*string(num_simulations) * ".jld2"
