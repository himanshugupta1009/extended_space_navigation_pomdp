...
function generate_prm_points_lookup_table_holonomic(world, graph)
    discretization_width_in_x = 1.0
    discretization_width_in_y = 1.0
    lookup_table = Array{Tuple{Int64,Float64,Float64,Float64},2}(undef,100,100)
    ...
    for i in 1:(world.length/discretization_width_in_x)
        for j in 1:(world.breadth/discretization_width_in_y)
            ...
            lookup_table[Int(i),Int(j)] = Tuple{Int64,Float64,Float64,Float64}((closest_prm_vertex,get_prop(graph,closest_prm_vertex,:x),
                                                                get_prop(graph,closest_prm_vertex,:y),get_prop(graph,closest_prm_vertex,:dist_to_goal)))
        end
    end
    return lookup_table
end
