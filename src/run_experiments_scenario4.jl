...
function run_experiment_pipeline(num_humans, num_simulations, write_to_file_flag = false)
    ...
    for iteration_num in 1:num_simulations
		...
		if(first_simulation_flag)
		    ...
		    if(lookup_table == nothing)
				graph = generate_prm_vertices(500, rand_noise_generator_for_prm, experiment_env)
	...
	results_filename = "./RESULTS/scenario4_humans_"*string(num_humans)*"_experiments_"*string(num_simulations) * ".jld2"
